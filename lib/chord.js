...
function Chord(root, intervals, bass, symbol) {
  if (!(this instanceof Chord)) return new Chord(root, intervals, bass, symbol);
...
Chord.prototype = {
  ...
  parallel: function() {
    ...
    if (this.chordType() !== 'triad' || !(quality === 'major' || quality === 'minor')) {
      throw new Error('Only major/minor triads have parallel chords');
    }

    if (this.isMajor()) {
      return new Chord(this.root.interval('m3', 'down'), 'minor');
    } else {
      return new Chord(this.root.interval('m3', 'up'), 'major');
    }
  },
  ...
  interval: function(interval) {
    return new Chord(this.root.interval(interval), this.kind, this.bass, this.degrees);
  },
...
Chord.fromString = function(str) {
  // N.C.
  if (str.match(/^\\s*N\\.?\\s*C\\.?\\s*$/i) || str.match(/^\\s*no\\s*chord/i))
    return new Chord(null, [], null, str);
  // Normal chords
  matches = str.match(/^\(?([A-Ga-gHh](?:x|#+|b+)?)\s*\(?((.*?)((?:(?:add|no|omit)[#b]*\d+|[#b]+\d+)*))\)?(?:\/([A-Ga-gHh][#b]*))?$/i);
  if (matches) {
    var root = Pitch.fromString(matches[1]);
    root.text = matches[1];
    ...
    var bass = matches[5] ? Pitch.fromString(matches[5]) : null;
    ...
    var intervalList = [];
    for (i in intervals) intervalList.push(Interval.fromString(intervals[i] + i));
    
    return new Chord(root, intervalList, bass, rest);
  }
}

Chord.fromXML = function(xml) {
  ...
  parser.parseString(xml, function(err, obj) {
    ...
    result = new Chord(root, kind, bass, degrees);
  });
  return result;
};
