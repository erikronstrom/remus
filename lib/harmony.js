...
Harmony.prototype = {
  ...
  init: function() {
    ...
    for (var i = 0; i < this.intervals.length; i++)
      this.intervals[i] = misc.initSubObject(this.intervals[i], this.env, Interval);
    return this;
  },
...
Harmony.coerce = function(source, env, copy) {
  ...
  if (source instanceof Chord) return new Harmony(source.harmony, env);
...
Harmony.fromString = function(str) {
  ...
  if (matches) {
    ...
    return new Harmony({root: root, intervals: intervalList, bass: bass, text: rest});
  }
}
