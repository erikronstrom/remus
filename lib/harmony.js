...
function Harmony(root, intervals, bass, symbol) {
  if (!(this instanceof Harmony)) return new Harmony(root, intervals, bass, symbol);
...
//     }
    return (this.root.text || (this.root.name().toUpperCase() + this.root.accidental()))
      + knowledge.harmonyShort[this.kind()] + bass;
  }
};
...
Harmony.fromString = function(str) {
  // N.C.
  if (str.match(/^\\s*N\\.?\\s*C\\.?\\s*$/i) || str.match(/^\\s*no\\s*harmony/i))
    return new Harmony(null, [], null, str);
  // Normal harmonies
  ...
  if (matches) {
    ...
    var intervals = {};
    var defaultIntervals = knowledge.harmonyDegrees[kind];
    ...
    return new Harmony(root, intervalList, bass, rest);
  }
}
...
Harmony.fromXML = function(xml) {
  ...
  parser.parseString(xml, function(err, obj) {
    ...
    result = new Harmony(root, kind, bass, degrees);
  });
  return result;
};
