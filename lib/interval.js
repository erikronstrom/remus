...
var Interval = Item.extend("Interval", ["coord"], {
  inspect: function() {
...
Interval.fromString = function(simple, env) {
  var coord = toCoord(simple);
  if (!coord)
    throw new Error('Invalid simple format interval');

  return new Interval({coord: coord}, env);
}

Interval.coerce = function(source, env, copy) {
  if (source instanceof Interval) {
    if (copy || (source.env !== env)) return new Interval(source, env);
    else return source;
  }
  if (typeof source === 'string') return Interval.fromString(source);
  if (source instanceof Array && source.length === 2) return new Interval(source, env);
