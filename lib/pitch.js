...
function Pitch(properties, env) {
  if (!(this instanceof Pitch)) return new Pitch(properties);
  this.mmel = properties.mmel;
  this.tcu = properties.tcu;
  this.coord = properties.coord;
  this.pitchDeviation = properties.pitchDeviation || 0;
...
Pitch.prototype = {
  inspect: function() {
    ...
    if (this.tcu) repr.push(color.bold(this.tcu) + ' ' + color('tcu[' + this.env.tcuPerOctave + ']'));
  ...
  name: function() {
    if (this.coord && this.env.stepsPerOctave % 7 === 0) {
      return knowledge.tones[(this.coord[0] % this.env.stepsPerOctave) / (this.env.stepsPerOctave / 7)];
    }
  },
  ...
  */
  ...
  toTcu: function() {
    ...
    if (isNumber(this.mmel)) return this.mmel * (this.env.tcuPerOctave / 12);
    if (this.coord) return this.coord[1];
  },
...
Pitch.coerce = function(source, env, copy) {
  ...
  if (typeof source === 'string') return Pitch.fromString(source, env);
  ...
  if (isNumber(source)) return new Pitch({tcu: source}, env);
